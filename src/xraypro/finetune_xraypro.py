...
class FinetuneXRayPro():
    """
    Training the model from scratch (no pretrained weights loaded)
    """
    def __init__(self, config, config_data, SEED = 0):
        self.config = config
        self.config_data = config_data
        self.model = loadModel(config = self.config, mode = 'cgcnn').regressionMode()
        ...
        self.train_loader, self.val_loader, self.test_loader = xraypro_loaders(directory_to_pxrd = self.directory_to_pxrd,
                                                                                directory_to_precursors = self.directory_to_precursors,
                                                                                directory_to_csv = self.path_to_csv,
                                                                                property = self.label,
